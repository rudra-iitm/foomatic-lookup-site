...
function getPrinterType(printer) {
    ...
    if (mechanism.dotmatrix !== undefined) return 'dot-matrix';

    if (mechanism.transfer === 'i') return 'inkjet';
    if (mechanism.transfer === 't') return 'laser';
...
async function combineData() {
    ...
    for (const [printerId, printer] of printers.entries()) {
        ...
        combinedPrinters.push({
            ...
            notes: printer.comments?.en || printer.comments || '',
        });
    }
    ...
    console.log(
        `Printers with drivers: ${
            combinedPrinters.filter((p) => p.drivers.length > 0).length
        }`
    );
}
